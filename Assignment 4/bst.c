...
struct BSTree*  newBSTree(){
	struct BSTree *mainTree = (struct BSTree *)malloc(sizeof(struct BSTree));
	initBSTree(mainTree);
	return mainTree;
}
...
/*recursive helper function to add a node to the binary search tree.
 ...
 param:  cur	the current root node
		 val	the val to be added to the binary search tree
 pre:	val is not null*/
...
/*function to add a val to the binary search tree
 param: tree   the binary search tree
		val		the val to be added to the tree

 pre:	tree is not null
	val is not null
 
pose:  tree size increased by 1
	tree now contains the val, val*/
...
/*function to determine if the binary search tree contains a particular element
 HINT: You have to use the compare() function to compare values.
 param:	tree	the binary search tree
		val		the val to search for in the tree
 pre:	tree is not null
		val is not null
 post:	none*/
/*----------------------------------------------------------------------------*/
...
/*helper function to find the left most child of a node
 return the val of the left most child of cur
...
/*recursive helper function to remove a node from the tree
 HINT: You have to use the compare() function to compare values.
 param:	cur	the current node
		val	the val to be removed from the tree
...
/*function to remove a val from the binary search tree
 param: tree   the binary search tree
	val		the val to be removed from the tree
 pre:	tree is not null
	val is not null
...
/* The following is used only for debugging, set to "#if 0" when used 
  in other applications */
	...
	void printNode(struct Node *cur) {
		 ...
		 printf("(");
		 printNode(cur->left);	 
		 /*Call print_type which prints the val of the TYPE*/
		 print_type(cur->val);
		 printNode(cur->right);
		 printf(")");
	}
...
struct BSTree *buildBSTTree() {
    ...
    */
	...
	/*Create val of the type of data that you want to store*/
