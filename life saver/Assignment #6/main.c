...
int main (int argc, const char * argv[]) {
    ...
    if(argc == 2)
        ...
        // filename = "/Users/seanmarty/Desktop/input1.txt"; /*specify your input text file here*/
        filename = "input1.txt";
    ...
    while (!feof(fileptr)) {
        if (!containsKey(hashTable, key, myCompare, hash2)) {
            insertMap(hashTable, key, (void*)1, myCompare, hash2);
        }
        else {
            
            insertMap(hashTable, key, (atMap(hashTable, key, myCompare, hash2) + 1), myCompare, hash2);
        }
    ...
    {
        key = nextMap(myItr);
        value = atMap(hashTable,key, myCompare, hash2);
